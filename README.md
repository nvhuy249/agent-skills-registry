...
- Accounts: signup/login with bcrypt-hashed passwords; user context kept in localStorage and passed via `user-id` header.
...
- Authentication is minimal (header-based + localStorage). With more time: add cookie-based JWT sessions (HttpOnly, SameSite=Lax) plus middleware to validate/attach user ID; remove `user-id` header usage; add input validation and light rate limiting on auth routes.
